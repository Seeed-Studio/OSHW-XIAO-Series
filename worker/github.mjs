...
export async function createSubmission(data, env, api) {
    ...
    const body = `A community project submission for review.\n\nProject details are included below and appended to projects.yaml. Before merging, apply the \`placement: landing page + project hub\` label to publish the project in both locations, or \`placement: project hub only\` to keep it in Project Hub. The selected label updates the catalog automatically.\n\n\`\`\`json\n${JSON.stringify(entry, null, 2).replace(/`/g, '\\u0060')}\n\`\`\`\n`;
    try {
