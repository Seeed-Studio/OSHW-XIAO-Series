...
export function inspectCatalogChange(baseText, headText, today = new Date().toISOString().slice(0, 10)) {
    ...
    for (const { project, index, added } of affected) {
        ...
        if (added && project.homepage === 'review') errors.push(`${label}: homepage review is pending; apply the placement: landing page + project hub or placement: project hub only PR label before merging.`);
        else if (added && !HOMEPAGE_DECISIONS.has(project.homepage)) errors.push(`${label}: apply the placement: landing page + project hub or placement: project hub only PR label before merging.`);
